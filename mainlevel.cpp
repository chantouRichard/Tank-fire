...
mainlevel::mainlevel(QWidget *parent)
    : QWidget(parent) {
...
void mainlevel::loadMap(int newMap[14][20]) {
    for (int i = 0; i < 14; ++i) {
        for (int j = 0; j < 20; ++j) {
            map[i][j] = newMap[i][j];
        }
...
void mainlevel::paintEvent(QPaintEvent *event) {
...
void mainlevel::initTank() {
    // 通用的初始化主坦克的逻辑
}

void mainlevel::initEnemyTank() {
    // 通用的初始化敌人坦克的逻辑
}
