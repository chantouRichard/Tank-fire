...
Level::Level(QWidget *parent)
    ...
    pixColumnTool1 = pixtool1.scaled(PropLabelSize, Qt::KeepAspectRatio);
    pixColumnTool2 = pixtool2.scaled(PropLabelSize, Qt::KeepAspectRatio);
    pixColumnTool3 = pixtool3.scaled(PropLabelSize, Qt::KeepAspectRatio);
    pixColumnTool4 = pixtool4.scaled(PropLabelSize, Qt::KeepAspectRatio);
...
void Level::updateTime() {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        switch (i) {
            case 0: propLabel = Prop1; break;
            case 1: propLabel = Prop2; break;
            case 2: propLabel = Prop3; break;
            case 3: propLabel = Prop4; break;
        }

        if (propLabel) {
            switch (PropType[i]) {
                case 0: propLabel->clear(); break;
                case 20: propLabel->setPixmap(pixColumnTool1); break;
                case 30: propLabel->setPixmap(pixColumnTool2); break;
                case 40: propLabel->setPixmap(pixColumnTool3); break;
                case 50: propLabel->setPixmap(pixColumnTool4); break;
            }
        }
    }
    update();
...
void Level::initTank(int enemy_num, int *enemysx, int *enemysy, int *styles, int my_tankx, int my_tanky) {
    // 初始化主坦克
    this->enemy_num = enemy_num;
    enemy_x_site = new int[enemy_num];
    enemy_y_site = new int[enemy_num];
    enemy_styles = new int[enemy_num];
    for (int i = 0; i < enemy_num; i++) {
        enemy_x_site[i] = enemysx[i];
        enemy_y_site[i] = enemysy[i];
        enemy_styles[i] = styles[i];
    }
...
void Level::initEnemyTank(int enemy_num) {
    ...
    for (int i = 0; i < enemy_num; i++) {
        ...
        enemys[i]->enemytank_style = enemy_styles[i];
...
void Level::HandleKey(int id) {
    if (id < 0) return;
    switch (PropType[id]) {
        case 20: my_tank->addLife(1); break;
        case 30: my_tank->shield(); break;
        case 40: my_tank->attack(); break;
        case 50: my_tank->speedUp(); break;
    }
    PropType[id] = 0;
}
