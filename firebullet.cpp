...
// 构造函数
firebullet::firebullet(QWidget *parent) : Bullet(parent)
{
    BULA = new QLabel; // 子弹标签
    BOOM = new QLabel; // 爆炸标签
    boomvideo = new QMovie(BulletBoom_gif); // 爆炸动画

    BULL.load(fire_bullet_pic); // 加载火焰子弹图片
...
// 检查子弹是否可以继续移动
bool firebullet::checkmovebullet()
{
    updatemapsit();
    emit fired(this->mapbuy, this->mapbux);
...
// 子弹爆炸处理
void firebullet::bulletboom(int boomsitx, int boomsity)
{
    BULA->hide();
    Disconnected();
    boomtimer->start(400);
...
// 检查子弹是否击中敌人
bool firebullet::checkshootenemy()
{
...
// 获取敌人位置
void firebullet::getenemysit(int x, int y)
{
    enemyx = x;
    enemyy = y;
}
